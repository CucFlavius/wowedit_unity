...
using UnityEngine;
using System.Linq;
using static DB2.DBDStructs;
using static DB2.DBDBuild;
using static DB2.DBDBuildRange;

public static partial class DB2
{
    public static class DBDReader
    {
        public static DBDefinition Read(string file, bool validate = false)
        {
            if (!File.Exists(file))
            {
                Debug.Log("Unable to find definitions file: " + file);
            }
            ...
            if (lines[0].StartsWith("COLUMNS"))
            {
                ...
                while (true)
                {
                    ...
                    if (!line.Contains(" "))
                    {
                        Debug.Log("Line " + line + " in file " + Path.GetFileNameWithoutExtension(file) + " does not contain a space between type and column name!");
                    }
                    ...
                    {
                        Debug.Log("Invalid type: " + type + " on line " + lineNumber);
                    }
                    else
                    {
                    ...
                    if (line.StartsWith(type + "<"))
                    {
                        ...
                        {
                            Debug.Log("Invalid foreign key length: " + foreignKey.Length);
                        }
                        else
                        {
                    ...
                    {
                        Debug.Log("Collision with existing column name while adding new column name! Skipping..");
                    }
                    else
                    {
            ...
            {
                Debug.Log("File does not start with column definitions!");
            }
            ...
            for (var i = lineNumber; i < lines.Length; i++)
            {
                ...
                if (!line.StartsWith("LAYOUT") && !line.StartsWith("BUILD") && !line.StartsWith("COMMENT") && !string.IsNullOrWhiteSpace(line))
                {
                    ...
                    {
                        Debug.Log("Unable to find " + definition.name + " in column definitions!");
                    }
                    else
                    {
            ...
            if (validate)
            {
                ...
                foreach (var column in columnDefinitionDictionary)
                {
                    ...
                    foreach (var version in versionDefinitions)
                    {
                        foreach (var definition in version.definitions)
                        {
                            if (column.Key == definition.name)
                            {
                                ...
                                {
                                    Debug.Log(Path.GetFileNameWithoutExtension(file) + "." + definition.name + " is called ID and might be a primary key.");
                                }
                                found = true;
                                break;
                            }
                    ...
                    if (!found)
                    {
                        Debug.Log("Column definition " + column.Key + " is never used in version definitions!");
                ...
                foreach (var version in versionDefinitions)
                {
                    foreach (var build in version.builds)
                    {
                        ...
                        {
                            Debug.Log("Build " + build.ToString() + " is already defined!");
                        }
                        else
                        {
                    ...
                    foreach (var layoutHash in version.layoutHashes)
                    {
                        ...
                        {
                            Debug.Log("Layout hash " + layoutHash + " is already defined!");
                        }
                        else
                        {
                        ...
                        {
                            Debug.Log("Layout hash \"" + layoutHash + "\" is wrong length for file " + file);
                        }
                    }
                    ...
                    foreach (var definition in version.definitions)
                    {
                        ...
                        {
                            Debug.Log("Version definition " + definition.name + " is an int/uint but is missing size in file " + file + "!");
                        }

                        if ((columnDefinitionDictionary[definition.name].type != "int" && columnDefinitionDictionary[definition.name].type != "uint") && definition.size != 0)
                        {
                            Debug.Log("Version definition " + definition.name + " is NOT an int/uint but has size in file " + file + "!");
                        }
                    }

                    if (version.definitions.GroupBy(n => n.name).Any(c => c.Count() > 1))
                    {
                        Debug.Log("Version definitions contains multiple columns of the same name!");
                    }
                }
                    ...
                    for (var j = 0; j < versionDefinitions.Count; j++)
                    {
                        ...
                        for (var b = 0; b < versionDefinitions[i].buildRanges.Length; b++)
                        {
                            for (var c = 0; c < versionDefinitions[j].builds.Length; c++)
                            {
                                ...
                                {
                                    Debug.Log("Build " + versionDefinitions[j].builds[c] + " conflicts with " + versionDefinitions[i].buildRanges[b] + "!");
                                }
                            }
                            ...
                            for (var c = 0; c < versionDefinitions[j].buildRanges.Length; c++)
                            {
                                ...
                                {
                                    Debug.Log("Build " + versionDefinitions[j].buildRanges[c] + " conflicts with " + versionDefinitions[i].buildRanges[b] + "!");
                                }
                            }
                        }
                        ...
                        if (versionDefinitions[i].definitions.SequenceEqual(versionDefinitions[j].definitions))
                        {
                            ...
                            {
                                Debug.Log(Path.GetFileNameWithoutExtension(file) + " has 2 identical version definitions (" + (i + 1) + " and " + (j + 1) + ") but two different layouthashes, ignoring...");
                            }
                            else
                            {
                                Debug.Log(Path.GetFileNameWithoutExtension(file) + " has 2 identical version definitions (" + (i + 1) + " and " + (j + 1) + ")!");
                            }
                        }
                    }
                }
